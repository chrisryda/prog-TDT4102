...
void RobotGrid::draw_grid_lines()
{
    ...
    int w_spacing = w_size/6;
    int h_spacing = h_size/6;
    for (int i = 0; i < 7; ++i) {
      window.draw_line(Point{x_pos+(i*w_spacing), y_pos}, Point{x_pos+(i*w_spacing), (y_pos+h_size)});
      window.draw_line(Point{x_pos, y_pos+(i*h_spacing)}, Point{(x_pos+w_size), y_pos+(i*h_spacing)});
    }
...
void RobotGrid::draw_robots()
{
  ...
  for (const auto & robot : robots) {
    Robot r = *(robot.second.get());
    window.draw_circle(get_grid_cell_center_coord(r.pos), (cell_width/2), r.color);
    window.draw_text(get_grid_cell_edge_coord(r.pos), r.name);
  }
...
void RobotGrid::check_coord_bounds(Point p) const
{
  ...
  if (p.x < 0 || p.x >= cols) {
    throw invalid_argument("x coordinate invalid");
  } else if (p.y < 0 || p.y >= rows) {
    throw invalid_argument("y coordinate invalid");    
  }
...
void RobotGrid::check_coord_empty(Point p, string name, bool is_moving) const
{
  ...
  for (const auto & robot : robots) {
    Robot r = *robot.second.get();
    if (r.pos.x == p.x && r.pos.y == p.y && !is_moving && r.name != name) {
      throw invalid_argument("Coordinate is occupied by another robot");
    }
  }
